...
- `api/resumo-dia.js`: status do time por **janela deslizante de 24h** (cron Vercel 21:00 UTC = 18:00 SP; auth `CRON_SECRET` no header ou `?s={ALERTAS_SECRET}`). Lê `lerResumo24h()` e enfileira "X enviadas / Y confirmadas" por contato, ordenado por envios (aviso `⚠️ volume alto` >=10). Janela de 24h (não dia calendário) para cobrir a noite anterior sem depender do dedupe mensal. Contagem via eventos: `ev:envio` (toda tentativa: foto/digitação/portal, `registrarEnvio`) e `ev:confirmada` (nota confirmada, `registrarConfirmada`, dedupe por chave). NÃO usar mais `alertaDia`/dia-calendário (o contador antigo era gated no dedupe mensal → nota repetida no mês não contava no dia; bug do "zerada" em 07/07).
- `api/reconciliar-fotos.js` + `lib/reconciliacao.js`: fecham o loop das fotos que deram TIMEOUT no OCR. `reconciliarFotos()` lê `fotosPendentes` (hash Redis, gravado no TIMEOUT), consulta cada imagem: OK → webhook confirma sozinho (só remove); INVALID/ERROR ou travada >15min → avisa "não consegui ler aquela foto, digite/site" e remove. Claim atômico (HDEL) evita aviso duplicado; espera 90s antes de intervir. Disparo: piggyback no `whatsapp-360.js` (toda mensagem) + endpoint (cron/QStash).
...
12. **TIMEOUT do OCR: sempre gravar `fotosPendentes` e deixar a reconciliação fechar o loop.** Se o polling estoura (45s), manda `msgProcessando` (copy honesta: pode ser a qualidade da imagem/QR, pode dar erro, se der certo avisa, e empurra digitar/portal), NUNCA a confirmação; e grava a foto pendente. `lib/reconciliacao.js` avisa o usuário quando o OCR falha/trava. O "não consegui ler / foto ruim" (`msgFotoNaoLida`) é só para `INVALID/ERROR` já no polling. Bug real (07/07): foto com QR ruim deu TIMEOUT e, como o webhook só dispara em OK, o usuário ficou sem retorno; a reconciliação resolve. NÃO remover o `registrarFotoPendente` do TIMEOUT.
13. **Número em Coexistence exige abrir o app Business a cada ≤13 dias.** Confirmado pelo 360dialog (13/07/2026): o número `5511980470391` está em Coexistence, e o app Business do celular precisa ser aberto pelo menos 1x a cada **13 dias**, senão a conta é DESATIVADA e o WABA cai do 360dialog (sem ninguém remover). Já caiu 2x por isso (celular ~20 dias desligado). Operação: manter o celular ligado/online e **abrir o app a cada ≤10 dias** (buffer), sem usá-lo para conversar (evita reentregas/fantasmas). Fix definitivo: migrar para Cloud API puro (desvincular da BM + do app Business, re-onboard na Cloud API) — remove a dependência do celular E as mensagens fantasma, mas tem downtime e re-registro (a `D360-API-KEY` muda; reconfigurar env/webhook), planejar fora de promoção.
