...
import { submeterChave, isValidChave } from './nota.js';
...
async function tratarNota(phone, msg, mediaId, session) {
  ...
  if (chave) {
    ...
    return submeterEResponder(phone, chave, session);
  }
...
async function tratarFoto(phone, mediaId, session) {
  ...
  // Aguarda o resultado do OCR (polling rápido)
  const resultado = await aguardarResultadoImagem(imageId);

  // Sucesso: extraiu a chave, agora submete AUTOMATICAMENTE na leitura de nota
  if (resultado.status === 'OK' && resultado.chave) {
    return submeterEResponder(phone, resultado.chave, session, true);
  }

  // Falha na leitura da foto: aplica fallback inteligente baseado no nº de tentativas
...
// ===== SUBMETER CHAVE E RESPONDER (compartilhado pelos 3 caminhos) =====
async function submeterEResponder(phone, chave, session, veioDefoto) {
  ...
  if (r.status === 'RECEIVED') {
    ...
    // Zera contador de tentativas de foto após sucesso
    await setSession(phone, { estado: 'aguardando_nota', tentativasFoto: 0 });
  } else if (r.status === 'DUPLICATED') {
    await sendText(phone, '🔁 Essa nota já tinha sido enviada antes!\n\nManda outra nota fiscal para continuar participando. 🎯');
    await setSession(phone, { estado: 'aguardando_nota', tentativasFoto: 0 });
