...
import { getSession, setSession, getUser, setUser, salvarLead, jaEnviouNota, contarNotasMes, associarChaveTelefone, enfileirarAlerta, registrarFotoPendente, registrarEnvio } from './store.js';
import { msgNotaNaFila, msgNotaJaNoSistema } from './mensagens.js';
...
// Extrai a chave de acesso (44 caracteres) de um texto digitado.
// Aceita chave numérica e a nova chave com CNPJ alfanumérico (letras no 7º-20º).
// A regex respeita a estrutura da chave (6 dígitos + 12 alfanuméricos do CNPJ + 26 dígitos)
// para NÃO confundir um texto/frase normal com uma chave (frase tem poucos dígitos seguidos).
function extrairChave(str) {
  const limpo = String(str).toUpperCase().replace(/[^0-9A-Z]/g, '');
  const m = limpo.match(/\d{6}[0-9A-Z]{12}\d{26}/);
  return m ? m[0] : null;
}
...
function textoComoEnviar(phone) {
  return 'Você pode mandar sua nota fiscal de *3 formas*:\n\n' +
    ...
    '*2.* 🔢 Digite os *44 números* da chave de acesso (ficam logo acima do QR code)\n\n' +
    '*3.* 🔗 Use nosso site: ' + linkPortal(phone);
}

// Foto que estourou o polling do OCR (TIMEOUT). Copy honesta: pode ser a qualidade da
// imagem/QR, pode demorar ou dar erro; se der certo o webhook confirma sozinho; e empurra
// o usuário para o caminho manual (digitar/portal), que é o mais confiável. A reconciliação
// (fotos pendentes) fecha o loop caso o OCR falhe de vez.
function msgProcessando(phone) {
  return '⚠️ *A qualidade da imagem e do QR code pode não estar boa.*\n\n' +
    'Ainda estou tentando ler o QR code e a chave de acesso. Isso pode demorar ou dar erro. Se der certo, eu te aviso aqui.\n\n' +
    'Mas o caminho mais rápido é você mesmo:\n' +
    '🔢 Digitar aqui no chat os *44 números* da chave de acesso (ficam logo acima do QR code)\n' +
    '🔗 Ou acessar o link, ler e enviar a nota por lá: ' + linkPortal(phone);
}

// Foto realmente ilegível (status INVALID/ERROR do OCR): a imagem não deu para ler.
// Aqui sim pedimos outra foto / digitar / site.
function msgFotoNaoLida(phone) {
  return '😕 *Não consegui ler o QR code dessa foto.*\n\n' +
    'Quase sempre é a foto: tremida, escura, com brilho/reflexo ou com o QR code cortado. A nota está ok, é só a imagem que ficou difícil de ler.\n\n' +
    'Vamos de novo? Escolha o caminho mais fácil pra você:\n\n' +
    '📷 Mande *outra foto* do QR code (firme, com boa luz, sem reflexo e com o código inteiro aparecendo)\n\n' +
    '🔢 Ou digite os *44 números* da chave de acesso (ficam logo acima do QR code)\n\n' +
    '🔗 Ou use o site: ' + linkPortal(phone) +
    '\n\nPode mandar quantas vezes precisar, não tem problema. 👍';
}
...
async function tratarNota(phone, msg, mediaId, session) {
  ...
  if (mediaId) {
    await registrarEnvio(phone); // conta o envio (acompanhamento do time), mesmo se a leitura falhar
  ...
  if (chave) {
    await registrarEnvio(phone); // conta o envio (digitação), mesmo se a chave não passar
    if (!isValidChave(chave)) {
      await sendText(phone, 'Essa chave não parece válida. 🤔\n\nConfira os *44 números* logo acima do QR code e tente de novo, ou mande uma *foto* da nota.');
      return;
    }
...
async function processarChave(phone, chave, session) {
  ...
  if (r.status === 'RECEIVED') {
    ...
    await sendText(phone, msgNotaNaFila(chaveReal));
    await setSession(phone, { estado: 'aguardando_nota', tentativasFoto: 0 });
    ...
    if (dados.ok) {
      ...
      await associarChaveTelefone(chaveReal, phone);
      await sendText(phone, msgNotaJaNoSistema(chaveReal));
    }
    await setSession(phone, { estado: 'aguardando_nota', tentativasFoto: 0 });
...
async function tratarFoto(phone, mediaId, session) {
  ...
  // Aguarda o resultado do OCR (polling síncrono até ~45s, ver lib/imagem.js)
  ...
  // TIMEOUT: o OCR não retornou a chave dentro do nosso limite de polling. Isso NÃO é
  // falha de leitura: a foto pode estar ótima e só o processamento/autorização estar
  // demorando (caso real: confirmou sozinho 15 min depois). Como a imagem foi enviada
  // com meta.wa, o webhook acha o dono e confirma quando ficar pronto. Por isso a copy é
  // "estou processando, te aviso aqui", e NÃO "não consegui ler" (que sugeria foto ruim).
  if (resultado.status === 'TIMEOUT') {
    await sendText(phone, msgProcessando(phone));
    // Guarda a foto como pendente: a reconciliação fecha o loop depois (confirma via
    // webhook se der certo, ou avisa que não deu se falhar/travar). Sem isso, foto que
    // falha no OCR deixa o usuário sem retorno.
    await registrarFotoPendente(imageId, phone);
    await setSession(phone, { estado: 'aguardando_nota', tentativasFoto: 0 });
    return;
  }

  // Falha REAL de leitura (INVALID/ERROR): a imagem não deu para ler. Aí sim pede outra
  // foto / digitar / site.
  await sendText(phone, msgFotoNaoLida(phone));
  await setSession(phone, { estado: 'aguardando_nota', tentativasFoto: 0 });
}
...
async function tratarChaveDaFoto(phone, chave, itemStatus, session) {
  ...
  if (itemStatus === 'RECEIVED') {
    // Nota nova: guarda associação chave->telefone como FALLBACK para o webhook achar o
    // dono. (A foto JÁ vai com meta.wa, ver tratarFoto, que é o caminho primário; o
    // chaveWa cobre o caso de o meta não voltar na nota.)
    await associarChaveTelefone(chave, phone);
    await sendText(phone, msgNotaNaFila(chave));
    await setSession(phone, { estado: 'aguardando_nota', tentativasFoto: 0 });
    ...
    if (dados.ok) {
      ...
      await associarChaveTelefone(chave, phone);
      await sendText(phone, msgNotaJaNoSistema(chave));
    }
    await setSession(phone, { estado: 'aguardando_nota', tentativasFoto: 0 });
