...
function msgNotaJaNoSistema(chave) {
  return '🧾 *Recebi sua nota!*\n\n' +
    linhaChave(chave) +
    'Ela já está no nosso sistema. Estou puxando os dados completos e te confirmo aqui em instantes. 🎯';
}

// Foto que não foi lida: pede um caminho mais garantido (a rede de segurança do
// webhook ainda confirma se o processamento terminar).
function msgFotoNaoLida(phone) {
  return '😕 Não consegui ler o QR code dessa foto.\n\n' +
    'Quase sempre é a foto: tremida, escura, com brilho ou com o QR code cortado. Vamos por um caminho mais garantido, escolha um:\n\n' +
    '📷 Mande *outra foto* com o QR code nítido e bem enquadrado\n\n' +
    '🔢 Ou digite os *44 números* da chave de acesso (ficam embaixo do QR code)\n\n' +
    '🔗 Ou leia pelo nosso site: ' + linkPortal(phone) +
    '\n\n(Se a leitura da foto ainda terminar, eu te confirmo aqui também.)';
}
...
async function tratarFoto(phone, mediaId, session) {
  ...
  // TIMEOUT: o OCR não retornou a chave no nosso limite. Na prática, isso costuma
  // indicar foto difícil de ler, então já tratamos como "não consegui" e oferecemos
  // um caminho mais garantido, em vez de deixar o usuário esperando. A imagem foi
  // enviada com meta.wa, então se o processamento terminar mesmo assim, o webhook
  // ainda acha o dono e confirma (a copy avisa isso entre parênteses).
