...
// O endpoint de imagem aceita os mesmos parâmetros do envio de chave padrão
// (multipart/form-data, campos como string). O meta enviado aqui é propagado
// até a fila de importação e volta na nota: é o que permite ao webhook achar
// o dono da foto por meta.wa, inclusive quando o OCR termina depois do polling.
export async function enviarImagemParaTera(buffer, mimeType, meta) {
...
// Faz polling do resultado da imagem até ~45s (dentro do maxDuration de 60s no vercel.json,
// com folga para o download da imagem, o POST do OCR e os envios).
// O OCR geralmente é rápido (poucos segundos), mas pode levar até ~1min em casos extremos.
// Aqui cobrimos o caso comum; se estourar, o chamador trata como "não consegui ler".
export async function aguardarResultadoImagem(imageId, tempoMaxMs, intervaloMs) {
  const tempoMax = tempoMaxMs || 45000;
