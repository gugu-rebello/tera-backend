...
// Valida formato da chave de acesso (44 caracteres, UF, modelo, DV).
// Aceita chave NUMÉRICA e a nova chave com CNPJ ALFANUMÉRICO (letras no 7º-20º caractere):
// o cálculo do DV usa o valor do caractere = ASCII menos 48 (padrão Serpro), que é
// idêntico ao numérico para dígitos. UF (1º-2º) e modelo (21º-22º) seguem numéricos.
export function isValidChave(str) {
  const clean = String(str || '').toUpperCase().replace(/[^0-9A-Z]/g, '');
  if (clean.length !== 44) return false;
  if (!/^[0-9]{2}$/.test(clean.substring(0, 2))) return false;
...
// Valor numérico do caractere para o módulo 11 (ASCII - 48): '0'..'9' -> 0..9, 'A'..'Z' -> 17..42.
function valorChar(c) { return c.charCodeAt(0) - 48; }

function validaDV(chave) {
  let soma = 0;
  let peso = 2;
  for (let i = 42; i >= 0; i--) {
    soma += valorChar(chave.charAt(i)) * peso;
    peso++;
    if (peso > 9) peso = 2;
  }
  const resto = soma % 11;
  const dv = (resto < 2) ? 0 : 11 - resto;
  return dv === valorChar(chave.charAt(43));
}
