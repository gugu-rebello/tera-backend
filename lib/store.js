...
export async function registrarNota(phone, accessKey) {
  try {
    ...
    // SADD dedupe a chave no mês. Usado só para o contador MENSAL que o participante vê
    // ("você já enviou X notas em julho"). O acompanhamento do time é separado (eventos 24h).
...
// ===== ACOMPANHAMENTO DO TIME: EVENTOS DAS ÚLTIMAS 24H =====
// Cada envio de nota e cada confirmação vira um evento com horário (ZSET, score = ts).
// O status do dia conta quem teve atividade nas últimas 24h, SEM depender do dedupe mensal
// (que fazia nota repetida no mês, comum em teste, não contar no dia). Enviadas: toda
// tentativa (member único). Confirmadas: por nota (member phone#chave, dedupe de retry).

const JANELA_MS = 24 * 60 * 60 * 1000;
const PODA_MS = 26 * 60 * 60 * 1000; // guarda um pouco além de 24h e poda o resto

export async function registrarEnvio(phone) {
  try {
    const ts = Date.now();
    const membro = phone + '#' + ts + '#' + Math.random().toString(36).slice(2, 8);
    await redis.zadd('ev:envio', { score: ts, member: membro });
    await redis.zremrangebyscore('ev:envio', 0, ts - PODA_MS);
  } catch (err) {
    console.error('erro registrarEnvio:', err.message);
  }
}

export async function registrarConfirmada(phone, accessKey) {
  try {
    const ts = Date.now();
    // member por nota: se o webhook reenviar, o ZADD só atualiza o score (não duplica).
    const membro = phone + '#' + accessKey;
    await redis.zadd('ev:confirmada', { score: ts, member: membro });
    await redis.zremrangebyscore('ev:confirmada', 0, ts - PODA_MS);
  } catch (err) {
    console.error('erro registrarConfirmada:', err.message);
  }
}

// Conta por telefone nas últimas 24h. Retorna { phone: { enviadas, confirmadas } }.
export async function lerResumo24h() {
  try {
    const agora = Date.now();
    const desde = agora - JANELA_MS;
    const envios = (await redis.zrange('ev:envio', desde, agora, { byScore: true })) || [];
    const confs = (await redis.zrange('ev:confirmada', desde, agora, { byScore: true })) || [];
    const mapa = {};
    for (const m of envios) {
      const phone = String(m).split('#')[0];
      if (!mapa[phone]) mapa[phone] = { enviadas: 0, confirmadas: 0 };
      mapa[phone].enviadas++;
    }
    for (const m of confs) {
      const phone = String(m).split('#')[0];
      if (!mapa[phone]) mapa[phone] = { enviadas: 0, confirmadas: 0 };
      mapa[phone].confirmadas++;
    }
    return mapa;
  } catch (err) {
    console.error('erro lerResumo24h:', err.message);
    return {};
  }
}
